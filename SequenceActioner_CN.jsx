...
// 序列帧动作控制脚本 v5 (中文版)
// 选中合成 → 读取图层 → 编辑信息 → 确定应用
// 在目标图层上添加时间重映射表达式、下拉菜单
// 帧数直接写入表达式，无需 Slider 控件
// 适配 AE 中文版：Menu="菜单", Slider="滑块"
// ================================================
(function() {
    ...
    // AE 2022 ExtendScript 可能缺少 String.trim，加 polyfill
    ...
    function buildExpression(menuName, actionNames, frameCounts, loopSettings) {
        var expr = "";
        expr += "var menu = effect(\"" + escapeExpressionText(menuName) + "\")(\"菜单\");\n";
    ...
    function execute() {
        ...
        try {
            ...
            var effects;
            try {
                effects = targetLayer.property("ADBE Effect Parade");
            } catch (e) {
                alert("步骤1失败: 获取效果器集合出错\n" + e.toString());
                return;
            }
            ...
            try {
                if (!hasDropdown) {
                    ...
                    if (foundIdx > 0) {
                        ...
                        if (!dropdownName) dropdownName = "下拉菜单控件";
                        try { effects.property(foundIdx).property(1).setValueAtTime(targetLayer.inPoint, 1); } catch (e) {}
                        try { effects.property(foundIdx).property("菜单").setValueAtTime(targetLayer.inPoint, 1); } catch (e) {}
                    }
                } else {
            ...
            try {
                ...
                alert("步骤4失败: 启用时间重映射出错\n" + e.toString());
                return;
            }

            if (!timeRemap) {
                alert("步骤4失败: 无法获取时间重映射属性\n请手动启用: 右键图层 > 时间 > 启用时间重映射");
                return;
            }
    ...
    // ================================================
    // 首帧智能对齐
    // 选中图层中最上方的一层为参考，只修改其余图层 Position。
    // 锚点始终不变。像素分析由同目录 SequenceAlignHelper.ps1 完成。
    // ================================================
    function quotePowerShellArg(value) {
        return "'" + String(value).replace(/'/g, "''") + "'";
    }

    // UTF-16LE 编码，避免 AE 的命令行桥接损坏中文源文件路径。
    ...
    function sourcePointToComp2D(layer, point) {
        // 使用明确的二维变换，避免 sourcePointToComp 在素材层返回 NaN。
    ...
    function smartAlignFirstFrames() {
        ...
        if (targets.length === 0) {
            alert("没有可对齐的目标图层。\n请确保图层是二维、无父级、未锁定，且 Position 没有关键帧或表达式。");
            return;
        }

        statusText.text = "正在分析首帧像素…";
        win.update();
        var layoutFile = new File(helperFile.parent.fsName + "/SequenceLayoutUtils.jsx");
        if (!layoutFile.exists) {
            alert("找不到尺寸适配辅助文件: " + layoutFile.fsName);
            return;
        }
        ...
        if (String(output).indexOf("ALIGN|") < 0) {
            alert("像素分析未返回有效结果。\n\n" +
                (String(output) || "辅助程序没有生成结果文件。") +
                "\n\n参考: " + referencePath + "\n辅助文件: " + helperFile.fsName);
            statusText.text = "";
